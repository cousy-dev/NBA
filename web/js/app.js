...
RENDERERS.scout = function () {
  ...
  const rows = sorted.map((r, i) => {
    ...
    return '<div class="scout-row' + (rep ? " reported" : "") + '">' +
      ...
      '    <span class="dr-name">' + esc(r.nameCn) + (r.isLegend ? ' <span class="dr-star">★</span>' : '') + (r.isLegend && !r.isCbaLegend ? ' <span class="nba-legend-tag">75大</span>' : '') + (r.isCbaLegend ? ' <span class="cba-tag">CBA</span>' : '') + ' <span class="pos-chip ' + posClass(r.pos) + '">' + esc(posLabel(r)) + "</span></span>" +
...
RENDERERS.draft = function () {
  ...
  $("#screen-draft").innerHTML =
    ...
    availFiltered.slice(0, SHOW).map((r, i) => {
      ...
      '  <span class="dr-rank">' + (i + 1) + "</span>" +
      '  <div class="dr-info">' +
      '    <div class="dr-name">' + esc(r.nameCn) + (r.isLegend ? ' <span class="dr-star">★</span>' : '') + (r.isLegend && !r.isCbaLegend ? ' <span class="nba-legend-tag">75大</span>' : '') + (r.isCbaLegend ? ' <span class="cba-tag">CBA</span>' : '') + ' <span class="pos-chip ' + posClass(r.pos) + '">' + esc(posLabel(r)) + "</span>" +
  ...
  m.innerHTML =
    '<div class="dpm-box">' +
      ...
      '<div class="dpm-player' + (isLegend ? " legend" : "") + '">' +
        '<div class="dpm-player-name">' + esc(rookie.nameCn) + (isLegend ? ' <span class="dpm-star">★</span>' : '') + (isLegend && !rookie.isCbaLegend ? ' <span class="nba-legend-tag">75大</span>' : '') + (rookie.isCbaLegend ? ' <span class="cba-tag">CBA</span>' : '') + '</div>' +
  ...
  $("#screen-draft-result").innerHTML =
    ...
    (myPicks.length ? '<div class="se-card gold"><h3>🎯 你的新秀</h3>' +
      myPicks.map(r =>
        ...
        '<div class="aw-name">' + esc(r.rookie.nameCn) + (r.rookie.isLegend && !r.rookie.isCbaLegend ? ' <span class="nba-legend-tag">75大</span>' : '') + (r.rookie.isCbaLegend ? ' <span class="cba-tag">CBA</span>' : '') + '<span class="aw-team">第' + r.pick + "顺位</span></div>" +
    ...
    firstRound.map(r =>
      ...
      '<div class="aw-name">' + esc(r.rookie.nameCn) + (r.rookie.isLegend && !r.rookie.isCbaLegend ? ' <span class="nba-legend-tag">75大</span>' : '') + (r.rookie.isCbaLegend ? ' <span class="cba-tag">CBA</span>' : '') + '<span class="aw-team">' + esc(teamName(r.abbr)) +
